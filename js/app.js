...
import { scanImage, variantsByBase } from "./pipeline.js?v20260616zaa";
import { T, LANGS, pickLang } from "./i18n.js?v20260616zaa";
...
const FEE = 1 / 1.15;
const NON_SELLABLE = new Set(["Common", "Uncommon", "Rare"]);   // grades that can't be listed on the Steam Market
...
function openPop(i, ev) {
  ...
  // Highlight 取引不可/無視 when this can't be listed: border hue unreadable, OR
  // a non-sellable grade (Common/Uncommon/Rare). Those have no matching catalog
  // option (only Immortal+ variants exist), so users hit a dead end in the
  // candidate list and reach for ignore — make that the obvious one-click action.
  $("popIgnore").classList.toggle("suggest", !c.border || NON_SELLABLE.has(c.border));
  // thumbnail
  const th = $("popThumb");
