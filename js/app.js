...
import { Matcher, _internal } from "./recognize.js?v20260626aa";
import { scanImage, variantsByBase } from "./pipeline.js?v20260626aa";
import { detectPageTab } from "./detect.js?v20260626aa";
import { putPage, deletePage, clearPages, loadPages, dbAvailable } from "./store.js?v20260626aa";
import { T, LANGS, pickLang } from "./i18n.js?v20260709o";
...
const CHANGELOG = [
  ...
  { v: "1.7.4", d: "2026/6/29",
    ...
    en: "Added an “⚖️ Exclude restricted” toggle to the coin spin-EV: switch to the value you can actually cash out now, dropping the 3 top grades still barred from listing." },
  { v: "1.7.3", d: "2026/6/28",
...
let GEX = localStorage.getItem("tbh_gex") === "1";   // gacha: drop the listing-restricted top grades from the spin EV
...
// MUST use the same population as gev or the two numbers aren't comparable. The
// bot's grade_averages() calls _real_unit() on its INTERNAL item dict, which has
// no "p" key (the ask lives in "usd" until write_snapshot converts it), so its
// ask branch is dead and only items carrying a median (m, else lm) are counted.
// Mirror that here: realUnit() would happily fall back to the bare ask and drag
// in the never-sold floor listings, which the mean above never saw.
...
function gradeMedians(mode) {
  ...
  for (const hash of Object.keys(DATA.items)) {
    ...
    let u;
    if (mode === "base") {
      u = unitPriceIn(hash, "base");            // baseline = one price per item, matches its mean
    } else {
      const p = DATA.prices?.items?.[hash];
      u = p ? (p.m ?? p.lm) : null;             // trusted median only — mirrors _real_unit
    }
    if (u == null) continue;
...
function renderGacha() {
  ...
  if (gex) {
    ...
    gex.onclick = () => {
      GEX = !GEX;
      try { localStorage.setItem("tbh_gex", GEX ? "1" : "0"); } catch (e) {}
      renderGacha();
    };
  }
